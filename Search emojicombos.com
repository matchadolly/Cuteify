...
emojidb = os.environ.get("emojidb")
...
def main():
    ...
    url = f"https://emojicombos.com/{url_query}"
    try:
        ...
        print(json.dumps({
            "items": [{
                ...
                {
                    "title": "Search using emojidb.org instead",
                    "arg": f"{emojidb} {url_query}",
                    "autocomplete": f"{emojidb} {url_query}",
                }]
        }))
        return
    ...
    results = soup.find_all("div", {"data-type": "emoji_combo"})
    items = []
    items.append({
